...
var AssessmentsList = (function (_super) {
    ...
    function AssessmentsList(props) {
        ...
        _this.renderList = function (rProps) {
            ...
            var content = children.map(function (item) {
                return React.cloneElement(item, { mode: 'list', onCancel: _this.handleItemCancel, itemClick: _this.handleItemClick, selectedId: null });
            });
            return React.createElement(GridList_1.GridList, { cellHeight: 180 }, content);
        };
        _this.renderAssessment = function (rProps) {
            var children = !Array.isArray(_this.props.children) ? [_this.props.children] : _this.props.children;
            var content = children.map(function (item) {
                return React.cloneElement(item, { mode: 'default', onCancel: _this.handleItemCancel, itemClick: _this.handleItemClick, selectedId: parseInt(rProps.match.params.id) });
            });
